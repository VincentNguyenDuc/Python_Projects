...
class Hearts:
    ...
    def __init__(self) -> None:
        self.target_score = self.int_input("Please enter a target score to end the game: ")
        self.number_of_players = self.int_input("Please enter the number of players (3-5): ", [3, 4, 5])
        self.game_play()
    ...
    def deck_generator(self):
        """
        This function returns a list of cards containing all 52 possible cards of a deck
    ...
    def player_generator(self):
        """
        This function returns a list of BasicAIPlayer objects.
    ...
    def dealing_cards(self, deck, players):
        """
        This function deal card to player hand. 
    ...
    def passing_card(self, players, round_tracker):
        """This method passes cards in the beginning of every round

        Args:
            players (list): a list of player objects
            round_tracker (int): track which round is it

        Returns:
            list: list of player objects with updated hands
        """
        ...
        for player in players:
            ...
            pass_list.append(card_pass)
        
        for i in range(len(players)):
            print(f"{players[i]} passed {pass_list[i]} to {players[i + pass_index - self.number_of_players]}")
    ...
    def game_play(self):
        """
        This method executes the whole game
        """ 
        condition = True
        round_tracker = 1
        ...
        while condition is True:
            ...
            players = self.dealing_cards(deck, players)
            players = self.passing_card(players, round_tracker)
            Round(players)
            print(f"========= End of round {round_tracker} =========")
            self.shoot_the_moon(players)
            self.round_points_display(players)
